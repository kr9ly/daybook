 ...
 *
 * [getDaybook] が返すインスタンスは本物の daybook の API 実装で動く: typed getter・edit の
 * バッチ・リスナーのセマンティクス・防御コピー — すべて本番とまったく同じに振る舞う。
 * 不在なのは永続層だけ: ファイルシステムには一切触れず、プラットフォームのランタイム
 * （Robolectric や実機）も不要。
 *
 * Android では同じ名前に SharedPreferences 互換 API（`getSharedPreferences`、actual 側で
 * 追加）もかぶせられる。同じ [name] は API をまたいで同一のストアを共有する — 1.x の
 ...
 * [IllegalArgumentException]（フラグとスキーマは API をまたいで検査される）。in-memory では
...
public expect class TestDaybook(packageName: String = "test") {
     ...
     * 同一性検査）。Android で同じ名前に SharedPreferences 互換 API が先に生成されていた場合も、
     ...
     * 書きアトミック性を保証する単位と同じ粒度。どの API（Daybook / SharedPreferences）からの
     ...
     * [Daybook.edit] は IOException を投げ、SharedPreferences 互換 API では `commit()` が
