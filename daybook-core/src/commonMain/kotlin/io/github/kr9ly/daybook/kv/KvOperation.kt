...
public sealed interface KvOperation {
     ...
     */
    public sealed interface Mutation : KvOperation

    /**
     * 単一キー空間への 1 操作。[Batch] の要素になれるのはこの型だけで、
     * バッチのネストやマーカーの混入を型で禁止する。
     */
    public sealed interface Single : Mutation
     ...
     */
    public data class Put(val key: String, val value: Any) : Single

    /** キーの削除。 */
    public data class Remove(val key: String) : Single

    /** 全キーの削除。 */
    public data object Clear : Single
     ...
     */
    public data class Batch(val operations: List<Single>) : Mutation
     ...
     */
    public data object SnapshotBoundary : KvOperation
}
